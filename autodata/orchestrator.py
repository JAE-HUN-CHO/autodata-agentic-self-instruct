...
class AgenticSelfInstruct:
    ...
    def _eval_solver(self, solver: Solver, qa: QAItem) -> list[float]:
        ...
        for _ in range(self.n_attempts):
            ...
            scores.append(self.judge.evaluate(qa, ans).score)
        return scores
    ...
    def run_paper(self, paper_id: str, paper_text: str) -> PaperResult:
        ...
        for round_no in range(1, self.max_rounds + 1):
            ...
            if not verdict.passed:
                ...
                self._log(f"  [{paper_id}] r{round_no}: FAILED_QV ({verdict.feedback})")
                continue
            ...
            if not wpass:
                ...
                self._log(f"  [{paper_id}] r{round_no}: TOO_EASY ({wmsg})")
                continue
            ...
            if not spass:
                ...
                self._log(f"  [{paper_id}] r{round_no}: FAILED_STRONG ({smsg})")
                continue
            ...
            if not gpass:
                ...
                self._log(f"  [{paper_id}] r{round_no}: FAILED_STRONG ({gmsg})")
                continue
            ...
            self._log(f"  [{paper_id}] r{round_no}: ACCEPTED "
                      f"(weak={mean(weak):.3f} strong={mean(strong):.3f} gap={gap:.3f})")
